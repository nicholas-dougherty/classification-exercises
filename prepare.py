...
#                F _ ", F _ ",    F ___J    F _ ",  /.\      F _ ",    F ___J                   #
#               J `-' |J `-'(|   J |___:   J `-' | //_\\    J `-'(|   J |___:                   #
#               |  __/F|  _  L   | _____|  |  __/F/ ___ \   |  _  L   | _____|                  #
#               F |__/ F |_\  L  F L____:  F |__// L___J \  F |_\  L  F L____:                  #
#              J__|   J__| \\__LJ________LJ__|  J__L   J__LJ__| \\__LJ________L                 #
#              |__L   |__|  J__||________||__L  |__L   J__||__|  J__||________|                 #
#|---------------------------------------------------------------------------------------------|#
#|---------------------------------------------------------------------------------------------|#
# IMPORTS
import pandas as pd
...
def prep_titanic(df):
    '''
    takes in a dataframe of the titanic dataset as it is acquired and returns a cleaned DF
    ...
    df = df.rename(columns={'embark_town_Queenstown': 'Queenstown', 'embark_town_Southampton':
                            'Southampton', 'sex_male': 'male'})
...
def prep_telco(df):
    ...
    df = df.drop(columns=['customer_id', 'payment_type_id', 'internet_service_type_id', 
                          'contract_type_id', 'total_charges', 'tech_support', 
...
def prep_titanic_keep_age(df):
    ...
    df_dummies = pd.get_dummies(df[['embark_town', 'sex']], dummy_na=False, drop_first=True)
    df = pd.concat([df, df_dummies], axis=1)
    df = df.rename(columns={'embark_town_Queenstown': 'Queenstown',
                            'embark_town_Southampton': 'Southampton', 'sex_male': 'male'})
